...
- [ ] **LOR-P001-050** — Define `HelpResult` struct: `captured_text: String`, `flag_used: HelpFlag (`Help` | `H` | `HelpSub`)`, `captured_at: jiff::Timestamp`, `exit_code: i32`, `pager_command: String` (the resolved cascade output)
...
- [ ] **LOR-P001-054** — Implement the spec §4.2.1 pager-selection cascade: `opts.pager` override (`--pager <cmd>`, with `""` meaning "disable") → `$MANPAGER` → `$PAGER` → `bat -pp` if `which("bat").is_ok()` → `cat`. Set the resolved value on both `PAGER` and `MANPAGER` in the subprocess env. Clear `LESS` only when the cascade fell back to the Steelbore default (`bat -pp`) or to `cat`; leave `LESS` untouched when honouring the user's pre-existing `$PAGER` / `$MANPAGER` so a user-tuned `LESS` (e.g. `-RFX`) still applies.
- [ ] **LOR-P001-054a** — Surface `HelpResult.pager_command` in the `--format json` envelope as `data.body.pager_command` (FR-024a).
- [ ] **LOR-P001-054b** — Wire `--pager <cmd>` as a `loran help` sub-command flag in `loran-cli`. `--pager=""` disables pagination (cat-equivalent).
...
- [ ] **LOR-P001-060a** — Add unit tests covering every step of the pager cascade: explicit `--pager <cmd>` override, `--pager=""` disable, `$MANPAGER` precedence over `$PAGER`, `$PAGER` fallback, `bat -pp` Steelbore default, and `cat` final fallback when `bat` is absent. Each test asserts the subprocess env actually receives the expected `PAGER` / `MANPAGER` / `LESS` values.
