...
|-----------------|----:|----------------------:|-------------------------------------------|
| Phase 0 — Setup |   5 |                    41 | ✅ Complete — 41 / 41 ticked              |
| Phase 1 — Ingot |  19 |                   146 | ✅ Complete — 143 / 146 ticked, 3 retired |
| Phase 2 — Billet|  18 |          *(stubs)*    | ✅ Complete — shipped without task-level decomposition; tagged `v0.2.0-billet` |
| Phase 3 — Bloom |   7 |          *(stubs)*    | ✅ Complete — shipped without task-level decomposition; tagged `v0.3.0-bloom` |
| **Total**       |  49 |                   187 | All four phases tagged                    |

The three retired Phase-1 tasks (LOR-P001-030/031/032) are page-list entries whose specifically-named tools were renormalised to a different 25-page catalog under maintainer priorities; equivalent category coverage shipped via the maintainer's-priorities escape hatch in LOR-P001-033.

Phase 2 and Phase 3 stubs were never expanded into per-task TODO entries; the work shipped against the Plan WPs directly. Future TODO revisions (v0.2 / v0.3) per the original schedule are no longer planned — the implementation is complete.
...
- [x] **LOR-P000-001** — Initialise git repository with `git init`
- [x] **LOR-P000-002** — Add `LICENSE` containing verbatim GPL-3.0-or-later text
- [x] **LOR-P000-003** — Add `.gitignore` excluding `target/`, `*.bak`, `.DS_Store`, `.idea/`, `.vscode/`, `node_modules/`, `*.swp`
- [x] **LOR-P000-004** — Add `.editorconfig` (UTF-8, LF endings, 4-space Rust, 2-space TOML/Markdown/YAML)
- [x] **LOR-P000-005** — Initial commit with DCO sign-off (`git commit -s`)

### WP-P0.02 — Posture files

**Sizing:** S | **Critical Path:** Yes | **Plan §:** 4 | **Deps:** WP-P0.01

- [x] **LOR-P000-006** — Create `README.md` skeleton: title, tagline, badges placeholder, section headings (Overview, Installation, Quickstart, Project Posture, Maintainer)
- [x] **LOR-P000-007** — Author README "Project Posture" section per Standard §5.1 (personal hobby, no SLA, GPL-3.0-or-later)
- [x] **LOR-P000-008** — Author README "Maintainer" section per Standard §13.2 (name, email, project URL, copyright year)
- [x] **LOR-P000-009** — Add README "Overview" with the three-questions framing from PRD §1
- [x] **LOR-P000-010** — Author `NOTICE.md` with no-warranty / no-liability statement deferring to GPL-3.0-or-later
- [x] **LOR-P000-011** — Author `CONTRIBUTING.md`: PR scope, DCO sign-off requirement, security-reporting path, license-of-contributions, maintainer-discretion (Standard §5.4)
- [x] **LOR-P000-012** — Cross-reference README → NOTICE, CONTRIBUTING, LICENSE via links
...
- [x] **LOR-P000-016** — Create workspace root `Cargo.toml` with `[workspace]` section listing 9 members (8 crates + `xtask`) and SPDX header
- [x] **LOR-P000-017** — Add `[workspace.dependencies]` table to root `Cargo.toml` with `serde`, `serde_json`, `toml`, `thiserror`, `anyhow`, `tracing`, `tracing-subscriber`, `jiff`, `clap`
- [x] **LOR-P000-018** — Create `rust-toolchain.toml` pinning to latest stable Rust (whatever it is at start of work)
- [x] **LOR-P000-019** — Create `rustfmt.toml` with `imports_granularity = "Crate"` and edition = 2024 (or current)
- [x] **LOR-P000-020** — Create `.clippy.toml` with project-specific lint config (e.g., `disallowed-methods = []` placeholder)
- [x] **LOR-P000-021** — Create `crates/loran-cli/` with `Cargo.toml`, `src/main.rs` (placeholder `fn main() { println!("loran") }`), SPDX headers
- [x] **LOR-P000-022** — Create `crates/loran-core/` with `Cargo.toml`, `src/lib.rs` (`pub fn placeholder() {}`), SPDX headers, `#![forbid(unsafe_code)]`
- [x] **LOR-P000-023** — Create `crates/loran-index/` with `Cargo.toml`, `src/lib.rs`, SPDX headers, `#![deny(unsafe_code)]`
- [x] **LOR-P000-024** — Create `crates/loran-pages/` with `Cargo.toml`, `src/lib.rs`, SPDX headers, `#![forbid(unsafe_code)]`
- [x] **LOR-P000-025** — Create `crates/loran-render/` with `Cargo.toml`, `src/lib.rs`, SPDX headers, `#![forbid(unsafe_code)]`
- [x] **LOR-P000-026** — Create `crates/loran-tldr/` with `Cargo.toml`, `src/lib.rs`, SPDX headers, `#![deny(unsafe_code)]`
- [x] **LOR-P000-027** — Create `crates/loran-tui/` with `Cargo.toml`, `src/lib.rs`, SPDX headers, `#![deny(unsafe_code)]`
- [x] **LOR-P000-028** — Create `crates/loran-mcp/` with `Cargo.toml`, `src/lib.rs`, SPDX headers, `#![deny(unsafe_code)]`
- [x] **LOR-P000-029** — Create `xtask/` with `Cargo.toml`, `src/main.rs` (placeholder), SPDX headers
- [x] **LOR-P000-030** — Verify `cargo build --workspace` succeeds with zero warnings
- [x] **LOR-P000-031** — Write a small shell script (POSIX `sh`) or `xtask` command that greps for SPDX headers in every `.rs` and `Cargo.toml`; verify it passes

### WP-P0.05 — Bootstrap CI pipeline

**Sizing:** M | **Critical Path:** No (but blocks confident merging) | **Plan §:** 4 | **Deps:** WP-P0.04

- [x] **LOR-P000-032** — Create CI workflow file (`.github/workflows/ci.yml` or equivalent for chosen forge)
- [x] **LOR-P000-033** — Add `cargo fmt --check` job step
- [x] **LOR-P000-034** — Add `cargo clippy --workspace --all-targets -- -D warnings` job step
- [x] **LOR-P000-035** — Add `cargo test --workspace` job step
- [x] **LOR-P000-036** — Add `cargo audit` job step (install `cargo-audit` if not pre-cached)
- [x] **LOR-P000-037** — Add SPDX-header check as a CI step calling the xtask command from LOR-P000-031
...
- [x] **LOR-P000-041** — Add CI status badge to top of `README.md`
...
- [x] **LOR-P001-001** — Add `serde`, `serde_derive`, `toml`, `thiserror` to `loran-pages/Cargo.toml` (use workspace dependencies)
- [x] **LOR-P001-002** — Define `Page` struct in `loran-pages/src/page.rs` mirroring spec §6.1 schema fields
- [x] **LOR-P001-003** — Implement `Deserialize` for the frontmatter struct via `serde` derive
- [x] **LOR-P001-004** — Implement frontmatter splitter: locate the `+++` fences, return (frontmatter_str, body_str)
- [x] **LOR-P001-005** — Implement TOML frontmatter deserialisation into the struct
- [x] **LOR-P001-006** — Implement required-field presence check (`name`, `category`, `summary`)
- [x] **LOR-P001-007** — Implement `summary` length validation (≤120 characters)
- [x] **LOR-P001-008** — Implement `safe_alias_for ⊆ replaces` invariant check; error includes the offending name
- [x] **LOR-P001-009** — Implement category-name well-formedness check (slash-tolerant; no leading/trailing slash; no double-slash)
- [x] **LOR-P001-010** — Define `PageError` enum with `thiserror`: `MissingFrontmatter`, `InvalidToml`, `MissingField(name)`, `SummaryTooLong`, `InvalidSafeAliasFor(name)`, `InvalidCategory(name)` and others as needed
- [x] **LOR-P001-011** — Implement `Page::parse(input: &str) -> Result<Page, PageError>` as the public entry point
- [x] **LOR-P001-012** — Add unit tests for 6+ valid pages (minimum viable, full schema, slash-category, multi-replaces, with-pairs-with, with-safe-alias-for)
- [x] **LOR-P001-013** — Add unit tests for each `PageError` variant (one test per failure mode)
- [x] **LOR-P001-014** — Add rustdoc comments to `Page`, `PageError`, `Page::parse`, and all public fields

### WP-P1.02 — Index loader + `Ingestor` trait (`loran-index`)

**Sizing:** M | **Critical Path:** Yes | **Plan §:** 5 | **Deps:** WP-P1.01 | **PRD:** FR-070

- [x] **LOR-P001-015** — Add `loran-pages` as a path dependency in `loran-index/Cargo.toml`
- [x] **LOR-P001-016** — Define the `Ingestor` trait with `fn ingest(&self) -> Result<Vec<Page>, IngestError>` in `loran-index/src/ingestor.rs`
- [x] **LOR-P001-017** — Define `IngestError` enum (`Io`, `Page(PageError)`, `BadSource`)
- [x] **LOR-P001-018** — Implement `MarkdownPagesIngestor` that takes a root directory and walks for `*.md`
- [x] **LOR-P001-019** — In `MarkdownPagesIngestor::ingest`, parse every found file via `loran-pages::Page::parse` and fail-loud on any error (no silent skipping per FR-035)
- [x] **LOR-P001-020** — Define `Index` struct holding primary `BTreeMap<String, Page>` keyed by `name`
- [x] **LOR-P001-021** — Add secondary indexes to `Index`: by-category, by-replaces (multimap), by-tag (multimap)
- [x] **LOR-P001-022** — Implement `Index::build(pages: Vec<Page>) -> Result<Index, IndexError>` with duplicate-name detection
- [x] **LOR-P001-023** — Implement lookup methods: `Index::get(name)`, `Index::by_category(cat)`, `Index::by_replaces(name)`, `Index::all()`
- [x] **LOR-P001-024** — Add unit tests for `Ingestor` (mock impl + verify trait constraints)
- [x] **LOR-P001-025** — Add unit tests for `MarkdownPagesIngestor` with a fixture directory tree
- [x] **LOR-P001-026** — Add unit tests for `Index` (build, lookup, duplicate-detection)
- [x] **LOR-P001-027** — Add rustdoc to `Ingestor`, `MarkdownPagesIngestor`, `Index`, and all public methods

### WP-P1.03 — Bundled pages tree (build-time)

**Sizing:** M | **Critical Path:** Yes | **Plan §:** 5 | **Deps:** WP-P1.01, WP-P1.02 | **PRD:** M-01, M-02

- [x] **LOR-P001-028** — Create top-level `pages/` directory and `pages/categories.toml` with initial categories: `file-listing`, `file-viewing`, `text-search`, `file-search`, `process-management`, `system-monitoring`, `networking`, `version-control`, `shell-utilities`, `data-processing`
- [x] **LOR-P001-029** — Author 5 curated pages for file-listing / file-viewing / text-search core trio: `eza.md`, `bat.md`, `rg.md`, `fd.md`, `dust.md`
- [ ] ~~**LOR-P001-030**~~ — *retired: page set renormalised under maintainer priorities. Of the originally-named pages, `procs.md`, `bottom.md`, and `hyperfine.md` shipped; `gping.md` and `xcp.md` did not. Equivalent system / process coverage delivered via `bandwhich.md`, `dust.md`, and `procs.md`.*
- [ ] ~~**LOR-P001-031**~~ — *retired: page set renormalised. `dog.md`, `xh.md`, `jaq.md`, and `sd.md` shipped; `gitui.md` did not. Git-TUI niche is covered by `lazygit.md` instead.*
- [ ] ~~**LOR-P001-032**~~ — *retired: page set renormalised. Only `starship.md` shipped from the original list; `nushell.md`, `ion.md`, `helix.md`, `zellij.md` deferred. The shell-utilities category is populated via `starship.md`, `direnv.md`, `just.md`, `zoxide.md`, and `hyperfine.md`.*
- [x] **LOR-P001-033** — Author 5+ additional pages covering the remaining 10 most-relevant tools to bring total to 25+ (`tealdeer`, `lsd`, `delta`, `tokei`, `bandwhich`, others per maintainer's catalog priorities)
- [x] **LOR-P001-034** — Verify all 20 legacy tools in PRD M-02 are represented in at least one page's `replaces` field
- [x] **LOR-P001-035** — Verify at least 10 pages include `pairs_with` entries (toward M-03 target of ≥1.5 average)
- [x] **LOR-P001-036** — Create `loran-core/build.rs` that reads `pages/` at compile time and emits a generated `BUNDLED_PAGES: &[(&str, &str)]` constant
- [x] **LOR-P001-037** — In `build.rs`, validate every page via `loran-pages::Page::parse`; fail the build on any error (compile-time validation per Plan §5)
- [x] **LOR-P001-038** — Implement `BundledPagesIngestor` in `loran-core` that wraps the `BUNDLED_PAGES` constant and implements the `Ingestor` trait
...
- [x] **LOR-P001-039** — Add `loran-index`, `loran-pages` as path deps in `loran-core/Cargo.toml`
- [x] **LOR-P001-040** — Add `nucleo-matcher` for fuzzy search
- [x] **LOR-P001-041** — Define `ShowResult` enum in `loran-core/src/show.rs`: `IndexHit { intro: String, body: ShowBody }`, `NoEntry { hint: String }`
- [x] **LOR-P001-042** — Define `ShowBody` enum: `Custom { page: Page, source_path: String }`, `None` (tldr variant deferred to Phase 2)
- [x] **LOR-P001-043** — Implement `resolve_show(index: &Index, tool: &str) -> ShowResult`
- [x] **LOR-P001-044** — Construct the no-entry hint string per spec §4.1.1 (includes `loran new <tool> --edit` and `see also: loran search <tool>`)
- [x] **LOR-P001-045** — Define `FindResult` and `resolve_find(index, query, alias_safe_only: bool) -> FindResult`
- [x] **LOR-P001-046** — Define `SearchResult` and `resolve_search(index, query) -> SearchResult` using `nucleo-matcher` over name/summary/replaces/tags
- [x] **LOR-P001-047** — Verify none of the three resolve functions panic on adversarial input (empty string, very long string, Unicode edge cases) — add fuzz-style tests
- [x] **LOR-P001-048** — Add unit tests for hit/miss/edge cases on all three resolve functions
...
- [x] **LOR-P001-049** — Add `which`, `jiff` deps to `loran-core/Cargo.toml`
- [x] **LOR-P001-050** — Define `HelpResult` struct: `captured_text: String`, `flag_used: HelpFlag (`Help` | `H` | `HelpSub`)`, `captured_at: jiff::Timestamp`, `exit_code: i32`, `pager_command: String` (the resolved cascade output), `pager_source: PagerSource` (one of `Flag`, `ManpagerEnv`, `PagerEnv`, `Bat`, `Moor`, `Cat`).
...
- [x] **LOR-P001-062** — Add `pulldown-cmark` to `loran-render/Cargo.toml`
- [x] **LOR-P001-063** — Implement `render_text(body_md: &str, writer: &mut impl Write) -> io::Result<()>` in `loran-render/src/text.rs`
- [x] **LOR-P001-064** — Handle `pulldown_cmark::Event` types: `Start/End(Heading)`, `Start/End(Paragraph)`, `Start/End(List)`, `Start/End(Item)`, `Start/End(CodeBlock)`, `Text`, `Code` (inline), `Link`, `Html` (passthrough as text)
- [x] **LOR-P001-065** — Render headings as plain text with capitalisation; render code blocks indented by 4 spaces; render lists with `-` bullets; render links as `text (url)`
- [x] **LOR-P001-066** — Ensure no ANSI escape codes appear in output (use a writer wrapper that strips them or simply emit none)
- [x] **LOR-P001-067** — Add unit tests round-tripping 4+ representative page bodies (paragraph-only, with-headings, with-code, with-list-and-links)
- [x] **LOR-P001-068** — Verify output passes through `grep '^[A-Z]'` and `awk '{print $1}'` on a sample without error
...
- [x] **LOR-P001-092** — Define `ExitCode` enum in `loran-cli/src/exit.rs` with all 12 named codes (0–5 canonical + 6–11 Loran-specific) per spec §9
- [x] **LOR-P001-093** — Implement `ExitCode::numeric() -> i32` and `ExitCode::name() -> &'static str`
- [x] **LOR-P001-094** — Implement `ExitCode::hint(context: &ErrorContext) -> String` per spec §12.3 (interpolates context like user's query for `NOT_FOUND`)
- [x] **LOR-P001-095** — Add unit tests confirming every variant produces a non-empty hint

### WP-P1.11 — `loran list` sub-command

**Sizing:** S | **Critical Path:** No | **Plan §:** 5 | **Deps:** WP-P1.02, WP-P1.07, WP-P1.08 | **PRD:** FR-001

- [x] **LOR-P001-096** — Implement `loran list` handler in `loran-cli/src/cmd/list.rs`
- [x] **LOR-P001-097** — Wire flags: `--category`, `--replaces`, `--safe-alias-for`, `--fields`
- [x] **LOR-P001-098** — Implement filter composition: filters apply in AND, not OR
- [x] **LOR-P001-099** — Implement `--fields name,summary` column projection
- [x] **LOR-P001-100** — Render text mode as a fixed-width table; render JSON mode as `data: [{...}, ...]` in the envelope
- [x] **LOR-P001-101** — Add integration tests for happy text, happy JSON, all four filter flags, and edge cases (empty result, unknown category)
...
- [x] **LOR-P001-102** — Implement `loran show` handler in `loran-cli/src/cmd/show.rs`
- [x] **LOR-P001-103** — Dispatch to `loran-core::resolve_show`; render `IndexHit` via `loran-render::render_text`
- [x] **LOR-P001-104** — On `NoEntry`, emit the no-entry diagnostic to stderr in text mode; emit `ErrorEnvelope` with `code = NOT_FOUND = 4` and hint in JSON mode
- [x] **LOR-P001-105** — Set `body.kind` to `"custom"` or `"none"` in JSON output (no `"tldr"` in Phase 1)
- [x] **LOR-P001-106** — Verify JSON output matches the spec §8 example structure
- [x] **LOR-P001-107** — Add integration tests for happy text, happy JSON, no-entry text, no-entry JSON
- [x] **LOR-P001-108** — Add `criterion` benchmark confirming cold-cache `loran show eza` completes in <50ms (NFR-001)
...
- [x] **LOR-P001-116** — Implement `loran find` handler in `loran-cli/src/cmd/find.rs`
- [x] **LOR-P001-117** — Wire `--safe-alias` flag for strict-mode filtering
- [x] **LOR-P001-118** — Dispatch to `loran-core::resolve_find`; sort results alphabetically by `name`
- [x] **LOR-P001-119** — Empty results → "no entries supersede `<legacy>`" diagnostic with hint `loran search <legacy>`
- [x] **LOR-P001-120** — Add integration tests for broad mode, strict mode, empty result, JSON envelope

### WP-P1.15 — `loran search` sub-command

**Sizing:** XS | **Critical Path:** No | **Plan §:** 5 | **Deps:** WP-P1.04, WP-P1.07 | **PRD:** FR-003

- [x] **LOR-P001-121** — Implement `loran search` handler in `loran-cli/src/cmd/search.rs`
- [x] **LOR-P001-122** — Dispatch to `loran-core::resolve_search`; preserve match-score ordering
- [x] **LOR-P001-123** — Empty result → "no matches for `<query>`" with hint `loran list --category=<...>` if a category-like word was in the query
- [x] **LOR-P001-124** — Add integration tests covering basic match, multi-word query, empty result, JSON envelope

### WP-P1.16 — `loran categories` sub-command

**Sizing:** XS | **Critical Path:** No | **Plan §:** 5 | **Deps:** WP-P1.02, WP-P1.07 | **PRD:** FR-002

- [x] **LOR-P001-125** — Implement `loran categories` handler in `loran-cli/src/cmd/categories.rs`
- [x] **LOR-P001-126** — Load `categories.toml` from the bundled tree; cross-reference with the index's by-category secondary index for counts
- [x] **LOR-P001-127** — Render text mode as `name | title | count` table; JSON mode as `data: [{name, title, description, count}]`
- [x] **LOR-P001-128** — Add integration tests for text + JSON output
...
- [x] **LOR-P001-133** — Add `schemars` to `loran-cli/Cargo.toml`
- [x] **LOR-P001-134** — Derive `JsonSchema` on the `Page` type in `loran-pages` (with `re-export` of the trait)
- [x] **LOR-P001-135** — Implement `loran schema` handler that emits the schema for `Page` only, with `meta.placeholder: true`
- [x] **LOR-P001-136** — Document the placeholder status in the sub-command's `--help` text
...
- [x] **LOR-P001-137** — Set up `loran-cli/tests/` with `assert_cmd`, `predicates`, `insta`, `tempfile`
- [x] **LOR-P001-138** — Add snapshot tests via `insta` for stable text-mode output of every sub-command
- [x] **LOR-P001-139** — Add JSON-mode validation tests for every sub-command (validate against schemars-derived schemas where available, hand-roll for others)
- [x] **LOR-P001-140** — Add error-path tests for every named exit code (6 through 9 are reachable in Phase 1)
- [x] **LOR-P001-141** — Add agent-env-var tests confirming TUI never activates with `AI_AGENT=1`
- [x] **LOR-P001-142** — Add `criterion` benchmark suite covering `loran show`, `loran list`, `loran search`
- [x] **LOR-P001-143** — Confirm NFR-001 (<50ms cold) passes for representative known tools
- [x] **LOR-P001-144** — Confirm NFR-002 (<100ms `loran list` for 1k catalog) passes
- [x] **LOR-P001-145** — Wire benchmark suite into CI with regression detection (fail on >10% slowdown)
- [x] **LOR-P001-146** — Run Standard §14 compliance checklist; record result in `compliance-log.md`
