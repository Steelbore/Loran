...
- [ ] **LOR-P001-050** — Define `HelpResult` struct: `captured_text: String`, `flag_used: HelpFlag (`Help` | `H` | `HelpSub`)`, `captured_at: jiff::Timestamp`, `exit_code: i32`, `pager_command: String` (the resolved cascade output), `pager_source: PagerSource` (one of `Flag`, `ManpagerEnv`, `PagerEnv`, `Bat`, `Moor`, `Cat`).
...
- [ ] **LOR-P001-054** — Implement the spec §4.2.1 pager-selection cascade: `opts.pager` override (`--pager <cmd>`, with `""` meaning "disable") → `$MANPAGER` → `$PAGER` → `bat -pp` if `which("bat").is_ok()` → `moor` if `which("moor").is_ok()` → `cat`. When `opts.skip_user_env_pager` is set (from the `--pager=loran` sentinel intercept), bypass the `$MANPAGER` and `$PAGER` steps. Set the resolved value on both `PAGER` and `MANPAGER` in the subprocess env. Clear `LESS` only when a Steelbore default-chain step (`bat -pp`, `moor`, or `cat`) fired; leave `LESS` untouched when honouring the user's pre-existing `$PAGER` / `$MANPAGER` so a user-tuned `LESS` (e.g. `-RFX`) still applies. Record which step won in `HelpResult.pager_source`.
- [ ] **LOR-P001-054a** — Surface `HelpResult.pager_command` and `HelpResult.pager_source` in the `--format json` envelope as `data.body.pager_command` and `data.body.pager_source` (FR-024a).
- [ ] **LOR-P001-054b** — Wire `--pager <cmd>` as a `loran help` sub-command flag in `loran-cli`. Two reserved sentinel values: `--pager=""` disables pagination (cat-equivalent); `--pager=loran` is intercepted at the CLI layer to set `HelpOpts.skip_user_env_pager = true` and pass `opts.pager = None`, forcing the Steelbore default chain (`bat -pp` → `moor` → `cat`). Document both sentinels in the `--help` text for `loran help`.
...
- [ ] **LOR-P001-059** — Implement `capture_help(tool: &str, opts: HelpOpts) -> Result<HelpResult, HelpError>` as the public entry point
